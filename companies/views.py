...
def list_view(request):
    columns = ['basic_cnpj', 'company_name', 'legal_nature', 'qualification_of_the_person_in_charge',
               'share_company_capital', 'company_size', 'federal_entity_responsible']
    ...
    if filter_column in columns and filter_value:
            companies = Company.objects.filter(**{f"{filter_column}__icontains": filter_value}).order_by('basic_cnpj')
    else:
        companies = Company.objects.all().order_by('basic_cnpj')
